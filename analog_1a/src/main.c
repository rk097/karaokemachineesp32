...
static int16_t delay[7];
static uint8_t idx = 0;

int16_t lowpass_7kHz(int16_t x) {
    delay[idx] = x;

    int32_t acc =  fir7[3] * delay[idx]                                 // h3·x[n-3]
                 + fir7[2] * (delay[(idx+6)%7] + delay[(idx+1)%7])      // h2·(x[n]+x[n-6])
                 + fir7[1] * (delay[(idx+5)%7] + delay[(idx+2)%7])      // h1· …
                 + fir7[0] * (delay[(idx+4)%7] + delay[(idx+3)%7]);     // h0· …
        ...
        if (xQueueReceive(adc_queue, &adc_data, portMAX_DELAY) == pdTRUE) {
            if (adc_data != NULL) {
                ...
                for (uint16_t i = 0; i < FRAME_SIZE; i += 2) {
                    ...
                    if (abs(i2s_data[i/2]) < 400) i2s_data[i/2] = 0; // noise gate
