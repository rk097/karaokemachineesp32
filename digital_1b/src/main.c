...
i2s_chan_handle_t i2s_in_handle;
i2s_chan_handle_t i2s_out_handle;
...
void i2s_out_task(void *param) {
    ...
    while (1) {
        if (xQueueReceive(i2s_queue_busy, &i2s_data, portMAX_DELAY) == pdTRUE) {
            ...
            if (xQueueSend(i2s_queue_busy, &i2s_data, portMAX_DELAY) != pdTRUE) {
...
void app_main(void) {
    ...
    for (uint8_t i = 0; i < DMA_BUFFER_COUNT; i++) {
        xQueueSend(i2s_queue_free, global_buffer[i], portMAX_DELAY);
    }
    ...
    printf("Threads started, main closing...\n");
}